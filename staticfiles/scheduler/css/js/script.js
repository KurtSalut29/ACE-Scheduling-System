...
document.addEventListener("DOMContentLoaded", () => {
  ...
  createMiniChart("chart_students", totals.students % 100, "#4e73df");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Bulk Actions
  // -----------------------------
  const applyBtn = document.getElementById("applyBulkAction"); // matches HTML
  ...
  applyBtn?.addEventListener("click", () => {
    ...
    fetch("", {
      ...
      body: JSON.stringify({ action: action, user_ids: selectedIds }),
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
