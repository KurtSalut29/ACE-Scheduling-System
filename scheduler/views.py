...
def manage_subjects(request):
    ...
    for subject in subjects.distinct():
        ...
        # Add metadata as dynamic attributes (use different names to avoid conflicts)
...
def manage_instructors(request):
    ...
    # Show instructors by department if selected
    if selected_department:
        instructors = (
    ...
    if request.method == "POST":
        ...
        if created:
            ...
            messages.info(request, f"{user.get_full_name()} already exists. Department updated to {department.name}.")
...
def manage_rooms(request):
    ...
    if selected_department:
        ...
        rooms_by_department[dept_name] = room_list
...
def manage_announcements(request):
    ...
    # 📢 Filter announcements by selected department (or show none if not selected yet)
    announcements = (
        Announcement.objects.filter(department=selected_department).order_by('-created_at')
        if selected_department
        else []
    )
